...
for (pipeline in c('SomaticSniper-Battenberg', 'SomaticSniper-TITAN','SomaticSniper-FACETS', 'MuTect-Battenberg','MuTect-TITAN', 'MuTect-FACETS')){
  # cnas_df <- read.table(paste0("~/isilon/private/users/asalcedo/baretrees/2020-06-15_",pipeline,"_Driver_CNAs_stacked.txt"),header=TRUE,stringsAsFactors =FALSE)
  # snvs_df <- read.table(paste0("~/isilon/private/users/asalcedo/baretrees/2020-06-15_",pipeline,"_Driver_SNVs_stacked.txt"),header=TRUE,stringsAsFactors =FALSE)
  trees_df <- read.table(paste0("~/isilon/private/users/asalcedo/baretrees/2020-06-15_", pipeline, "_baretrees_brca_singles.txt"),header=FALSE,stringsAsFactors =FALSE, fill=TRUE)
  print(pipeline)
...
trees$V1[trees$V1 == "CPCG0260.B1F1"] <- "CPCG0260"
  ...
  # for (sample in c("CPCG0575", "CPCG0166")){
    ...
    for (Vcaller in Vcallers){
      ...
      if(any(tree_heights[,3] > 4.5)){
        scale.x.real <- 1/90
        pga.interval <- 20
        if(any(tree_heights[,4] > 3.2)){
          scale2 <- 0.5/2000
          snv.interval <- 2000 
        }
      } else if(any(tree_heights[,3] > 3.5)){
        scale.x.real <- 1/60
        pga.interval <- 20
      ...
      # while(length_diff > 0.01){
            for(Ccaller in Ccallers ){
              ...
              tree_height <- data.frame(PGA=sum(as.numeric(samp_tree$V9),na.rm=TRUE)*scale.x.real, SNV=sum(as.numeric(samp_tree$V4),na.rm=TRUE)*scale2)
              if(max(tree_height[1,]) < 0.01){
                if(tree_height[1,1] < 0.01 & sum(as.numeric(samp_tree$V9),na.rm=TRUE) > 0){
                  scale.x.real <- 1/15
                  pga.interval <- 5
                }
                if(tree_height[1,2] < 0.01 & sum(as.numeric(samp_tree$V4),na.rm=TRUE) > 0){
                  scale2 <- 0.5/750
                  snv.interval <- 500
                }
              }
              ...
              if( nrow(samp_tree)==0){ #no trees
                ...
                if(length(unique(paste(trees_df$V_caller, trees_df$C_caller))) == 4){
                  ...
                  }else if( length(unique(paste(trees_df$V_caller, trees_df$C_caller))) == 6){
                    # if(length(unique(Vsamp_tree$C_caller))==3){                    
                    #   axis.type <- if(Ccaller == 'Battenberg')'PGA' else if(Ccaller == "TITAN") 'SNV' else "none"
                    #   }else{
                        axis.type <- "both"
                      # }
                  ...
                  axis.type <- "both"
                if(length(Vsamp_tree$V2[Vsamp_tree$V2==0]) > 3){
                  axis.type="both"
                }
    ...
    #  d_ply(samp_tree, .(V1,pipeline), function(x) {non_bin <- any(sapply(unique(x$V2), function(p) length(x$V2[x$V2==p])>2) == TRUE); if(non_bin == TRUE){print(x)}})              
               ...
               # source('~/cluster/svn/Collaborators/BrendaGallie/Rb1_follow_up/WGS/subclone_plotting/position_clones.R')
                ...
                out <- draw.my.tree2(in.tree.df=inputs$in.tree.df,tree=inputs$tree,genes.df= inputs$genes.df,filename="scrap.pdf",scale2=scale2,wid=.8,extra.len=extra.len,len=10,
                       scale.x.real = scale.x.real,  w.padding = inputs$w.padding, h.padding=1, offset=0.0467,axis.type=axis.type,rad=0.08,gene.cex=1.22, lab.cex=1.1, axis.cex=1, title.cex=1.2,node.cex=1.2,
                       add.genes=FALSE,add.circle=TRUE, add.labels=TRUE,seg1.col='navy',seg2.col='gold',samp_name=inputs$samp.name,circle.col="grey20",lin.width=1,curve=3,
                ...
                # if(Ccaller == "Battenberg" & Vcaller == "MuTect"){break}
      ...
      test_ag <- arrangeGrob(titleGrob, pipe_trees[['SomaticSniper-Battenberg']],pipe_trees[['SomaticSniper-FACETS']], pipe_trees[['SomaticSniper-TITAN']], pipe_trees[['MuTect-Battenberg']],pipe_trees[['MuTect-FACETS']], pipe_trees[['MuTect-TITAN']],
          ncol=3,nrow=3,padding=unit(0.25,"inches"),heights=unit(c(0.1,0.4,0.5),"npc"),layout_matrix=rbind(c(1,1,1),c(2,3,4),c(5,6,7)))
      
        # pdf(paste0("~/test.pdf"),onefile=FALSE,height=10, width=8)
        pdf(paste0("~/isilon/private/users/asalcedo/Lydia_trees_revised/brca_single/",sample,".pdf"),onefile=FALSE,height=8.5, width=11)
        grid.newpage()
...
source('~/cluster/svn/Collaborators/BrendaGallie/Rb1_follow_up/WGS/subclone_plotting/draw.my.tree2.R')
      ...
      pdf("~/test.pdf",onefile=FALSE,height=10, width=8)
      grid.newpage()
      pushViewport(viewport())
...
cnas <- read.table("~/isilon/private/users/asalcedo/baretrees/2020-06-15_SomaticSniper-Battenberg_Driver_CNAs_stacked.txt",header=TRUE,stringsAsFactors =FALSE)
trees <- read.table("~/isilon/private/users/asalcedo/baretrees/2020-06-15_SomaticSniper-Battenberg_baretrees.txt",header=FALSE,stringsAsFactors =FALSE, fill=TRUE)
snvs <- read.table("~/isilon/private/users/asalcedo/baretrees/2020-06-15_SomaticSniper-Battenberg_Driver_SNVs_stacked.txt",header=TRUE,stringsAsFactors =FALSE)
names(genes)[3] <- "cn"
...
cnas <- cnas[which(!(is.na(cnas$cn) & is.na(cnas$node))),]
...
source('~/cluster/svn/Collaborators/BrendaGallie/Rb1_follow_up/WGS/subclone_plotting/add.text2.R')
out <- draw.my.tree2(in.tree.df=inputs$in.tree.df,tree=inputs$tree,genes.df= inputs$genes.df,filename=inputs$out.name,scale2=0.5/362,wid=1.1,extra.len=20,len=10,
             scale.x.real = 1/10, sig.shape=2.5, w.padding = inputs$w.padding+0.5, h.padding=1, offset=1.4,axis.type=axis.type,rad=0.1,gene.cex=1.22,
