...
for (pipeline in c('SomaticSniper-Battenberg', 'SomaticSniper-TITAN','SomaticSniper-FACETS', 'MuTect-Battenberg','MuTect-TITAN', 'MuTect-FACETS')){
  cnas_df <- read.table(paste0("~/isilon/private/users/asalcedo/baretrees/",pipeline,"/2020-06-15_Driver_CNAs_stacked.txt"),header=TRUE,stringsAsFactors =FALSE)
  snvs_df <- read.table(paste0("~/isilon/private/users/asalcedo/baretrees/",pipeline,"/2020-06-15_Driver_SNVs_stacked.txt"),header=TRUE,stringsAsFactors =FALSE)
  trees_df <- read.table(paste0("~/isilon/private/users/asalcedo/baretrees/",pipeline,"/", pipeline, "_baretrees.txt"),header=FALSE,stringsAsFactors =FALSE, fill=TRUE)
  print(pipeline)
  ...
  # for (sample in c("CPCG0575", "CPCG0166")){
    ...
    #  d_ply(samp_tree, .(V1,pipeline), function(x) {non_bin <- any(sapply(unique(x$V2), function(p) length(x$V2[x$V2==p])>2) == TRUE); if(non_bin == TRUE){print(x)}})              
      ...
      test_ag <- arrangeGrob(titleGrob, pipe_trees[['SomaticSniper-Battenberg']],pipe_trees[['SomaticSniper-TITAN']], pipe_trees[['MuTect-Battenberg']],pipe_trees[['MuTect-TITAN']],
        ...
        pdf(paste0("./Lydia_trees_revised/",sample,".pdf"),onefile=FALSE,height=10, width=8)
        grid.newpage()
...
trees <- read.table("~/isilon/private/users/asalcedo/baretrees/2020-06-15_SomaticSniper-Battenberg_baretrees.txt",header=FALSE,stringsAsFactors =FALSE)
