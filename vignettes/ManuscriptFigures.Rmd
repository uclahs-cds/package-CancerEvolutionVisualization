...
# Migration path tree
mig.dt <- data.frame(
    parent = c(NA, 'R1', 'LN1', 'LN1'),
    label = c('R1', 'LN1', 'LN2', 'LN3'),
    node.col = spatial.col[c('Primary', 'Metastasis', 'Metastasis', 'Metastasis')]
    );
rownames(mig.dt) <- mig.dt$label;
...
# Define a viewport for plt2 inside the plot space
vp.plt2 <- viewport(
    x = 0.85,
    y = 0.8,
    width = 0.15,
...
pushViewport(vp.plt2);
# Draw a black border (rectangle) around the viewport
