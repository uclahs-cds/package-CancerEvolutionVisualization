...
multiregion_ccfs <- function(tree_in, ccf_names, tumour_name, tree_gTree,width=1.5, nrow=2, ncol=2){
	...
	if(is.null(ccf_names)){
		ccf_names <- paste("ccf", seq_along(samps), sep="_")
	}

	in.df <- separate(col=ccf, sep=",",into=ccf_names, in.df, convert=TRUE)
	in.df$fill <- tree_in$fil
	...
	for (i in seq_along(samps)){
		...
		test_tree <- gTree(children=gList(textGrob(ccf_names[i],x=unit(0.5,"npc"),y=unit(1.3,"npc"),gp=gpar(cex=0.8))), 
	...
	test <- arrangeGrob(grobs=samp_list, nrow=nrow, ncol=ncol, vp=viewport(height=unit(4,"inches"),y=0.5,x=0.5,clip="off"), clip="off")

	test <- gtable_add_grob(test, grobs = rectGrob(gp = gpar(fill = 'white', col="white", lwd = 0)), 1, 1, nrow(test), ncol(test), 0);
