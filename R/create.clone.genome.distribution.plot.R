...
plot.clone.genome.distribution.per.sample <- function(
    ...
    for (c in unique(sample.df$clone.id)) {
        print(c);
        print(sum(sample.df$clone.id == c) <= 1);
        if (sum(sample.df$clone.id == c, na.rm = TRUE) <= 1) {
            warning(paste('Skipping clone', c, 'in sample', unique(sample.df$ID), 'since there is <= 1 SNV'));
            next;
        }
    ...
    scatter.plt <- create.clone.genome.distribution.scatter(
    ...
    density.plt <- create.clone.genome.distribution.density(
        density.df,
        cluster.colours,
        chr.info
        );
