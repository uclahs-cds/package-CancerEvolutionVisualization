...
add_clone_grobs <- function(clone.out, ...){
    ...
    for (j in 1:length(clone.out$clones)){
      polygon_list[[j]] <- polygonGrob(name = paste0('clone.polygon.', j), x=clone.out$clones[[j]]$x, y= clone.out$clones[[j]]$y, default.units="native", gp=gpar(fill=clone.out$clones[[j]]$col,col='transparent',alpha=clone.out$clones[[j]]$alpha))
    }
